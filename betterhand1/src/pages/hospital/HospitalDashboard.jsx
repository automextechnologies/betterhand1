    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="card w-full max-w-md animate-scale-in">
        <div className="flex items-center justify-between p-5 border-b border-surface-200">
          <h2 className=" font-bold text-surface-900">Complete Donation</h2>
          <button onClick={onClose} className="p-2 hover:bg-surface-100 rounded-lg text-surface-400">✕</button>
        </div>
        <div className="p-5 space-y-4">
          <div className="p-3 rounded-xl bg-brand-950/40 border border-brand-700/30">
            <p className="text-sm text-surface-800  font-semibold">{response.donor_name}</p>
            <p className="text-xs text-surface-500 mt-0.5">{response.blood_group} donor</p>
          </div>
          <div>
...
export default function HospitalDashboard() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button onClick={clearAllData} disabled={clearing}
            className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm  font-semibold
                       bg-danger-600/10 text-danger-400 border border-danger-600/20
      ...
      {/* Tabs */}
      <div className="flex gap-1 p-1 rounded-xl bg-surface-50 border border-surface-200 w-fit">
        {[
          ...
          <button key={t.id} onClick={() => setActiveTab(t.id)}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm  font-semibold transition-all
              ${activeTab===t.id ? 'bg-gradient-to-r from-brand-600 to-brand-400 text-surface-800 shadow-md' : 'text-surface-400 hover:text-surface-600'}`}>
      ...
      {activeTab === 'requests' && (
        loading ? (
          <div className="flex justify-center py-10"><Spinner size={28} className="text-brand-400"/></div>
        ) : activeRequests.length === 0 ? (
          <div className="card p-10 text-center">
            <Droplets size={32} className="text-surface-200 mx-auto mb-3"/>
            <p className="text-surface-400">No active requests.</p>
                ...
                <div key={req.id} className="card overflow-hidden">
                  ...
                  <div className="flex items-center gap-4 p-4 cursor-pointer hover:bg-surface-50 transition-colors"
                    ...
                    <div className="w-14 h-14 rounded-2xl bg-blood flex items-center justify-center shrink-0">
                      <span className="font-mono font-bold text-danger-300 text-lg">{req.blood_group}</span>
                    </div>
                    ...
                    {isOpen ? <ChevronUp size={16} className="text-surface-300 shrink-0"/> : <ChevronDown size={16} className="text-surface-300 shrink-0"/>}
                  </div>
                  ...
                  {isOpen && (
                    <div className="border-t border-surface-200 p-4">
                      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                        ...
                        {/* Top 3 */}
                        <div>
                          <h4 className=" font-semibold text-surface-600 text-sm mb-3">Top 3 Donors</h4>
                        ...
                        {/* Map + confirmed list */}
                        <div>
                          <h4 className=" font-semibold text-surface-600 text-sm mb-3">Live Tracking</h4>
                          <LiveMap
                              ...
                              {confirmed.map((d, i) => (
                                <div key={d.id} className="card p-3">
                                  <div className="flex items-center gap-2 mb-2">
                                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold text-surface-900
                                      ${['bg-emerald-600','bg-amber-600','bg-sky-600'][i]}`}>{i+1}</div>
                                    <span className="text-surface-700 text-sm font-semibold flex-1">{d.donor_name}</span>
                                  ...
                                  <div className="flex gap-1.5 mb-2">
                                    ...
                                    {d.donor_whatsapp && (
                                      <a href={whatsappLink(d.donor_whatsapp)} target="_blank" rel="noreferrer"
                                        className="flex-1 flex items-center justify-center gap-1 py-1.5 rounded-lg bg-accent-600/20 text-accent-400 text-xs font-semibold border border-accent-600/20">
                                  ...
                                  {d.status === 'completed' && (
                                    <div className="text-center py-1.5 text-xs text-accent-400 font-semibold">
                                      ✅ Donation Completed
                                    </div>
                                  )}
                                </div>
                        ...
                        <div>
                          {confirmed.length > 0 ? (
                            ...
                            <div className="card h-72 flex items-center justify-center text-center p-6">
                              <div>
                                <Clock size={24} className="text-surface-200 mx-auto mb-2"/>
                                <p className="text-surface-400 text-sm">Chat available after confirming donors</p>
                              </div>
                            </div>
                          )}
                        </div>
