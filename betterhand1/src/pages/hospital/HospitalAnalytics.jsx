...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="glass rounded-xl px-3 py-2 border border-surface-200 text-xs">
...
export default function HospitalAnalytics() {
  ...
  if (!data) return <p className="text-surface-400 text-center py-20">No analytics data.</p>
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Requests', val: data.total_requests, icon: Droplets, color: 'text-brand-400' },
          { label: 'Success Rate', val: `${data.success_rate_percent}%`, icon: TrendingUp, color: 'text-accent-600' },
          ...
          <div key={k.label} className="stat-card animate-fade-up">
            ...
            <p className="text-3xl  font-bold text-surface-900 mt-2">{k.val}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card p-5">
          <h3 className=" font-semibold text-surface-900 mb-4">Monthly Donations</h3>
        ...
        {/* By blood group */}
        <div className="card p-5">
          <h3 className=" font-semibold text-surface-900 mb-4">Donations by Blood Group</h3>
          <ResponsiveContainer width="100%" height={200}>
        ...
        {/* By urgency */}
        <div className="card p-5">
          <h3 className=" font-semibold text-surface-900 mb-4">Requests by Urgency</h3>
            ...
            <div className="space-y-2">
              {(data.by_urgency || []).map((u, i) => (
                <div key={u.urgency} className="flex items-center gap-2 text-sm">
                  ...
                  <span className="text-surface-600 capitalize">{u.urgency}</span>
                  <span className="text-surface-900 font-mono font-medium ml-auto">{u.count}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* By status */}
        <div className="card p-5">
          <h3 className=" font-semibold text-surface-900 mb-4">Requests by Status</h3>
              ...
              return (
                <div key={s.status}>
                  <div className="flex justify-between mb-1">
                    <span className="text-sm text-surface-600 capitalize">{s.status}</span>
                    <span className="text-sm font-mono text-surface-900">{s.count}</span>
                  </div>
