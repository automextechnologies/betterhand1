...
function BloodGroupBadge({ group }) {
  return <span className="bg-blood px-3 py-1.5 rounded-xl font-mono font-bold text-danger-300 text-lg">{group}</span>
}
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="card p-5 flex items-center gap-4">
        <div className="w-14 h-14 rounded-2xl bg-gradient-to-r from-brand-600 to-brand-400 flex items-center justify-center text-xl font-bold text-surface-900 shrink-0">
          {profile?.full_name?.[0] || user?.email?.[0]?.toUpperCase()}
        </div>
        <div className="flex-1">
          <h1 className=" text-xl font-bold text-surface-900">{profile?.full_name || 'Donor'}</h1>
        ...
        <div className="text-right">
          <p className="text-3xl  font-bold text-surface-900">{history.length}</p>
          <p className="text-xs text-surface-400">Total Donations</p>
        </div>
      </div>
            ...
            {badges.map(b => {
              ...
              return (
                <div key={b.badge} className="glass px-3 py-2 rounded-xl flex items-center gap-2 text-sm">
                  ...
                  <span className=" font-medium text-surface-900">{labels[b.badge]}</span>
                </div>
              )
            })}
          </div>
      ...
      <div>
        ...
        {pending.length === 0 ? (
          <div className="card p-8 text-center">
            ...
            <p className="text-surface-400">No pending blood requests right now.</p>
            <p className="text-surface-300 text-xs mt-1">You'll be notified when a hospital needs your blood type.</p>
          </div>
        ) : (
              ...
              <div key={resp.id} className={`card p-4 animate-fade-up ${resp.via_ward ? 'border-2 border-brand-500/50 shadow-lg' : 'border border-brand-700/30 shadow-md'}`}>
                <div className="flex items-start gap-3 mb-3">
                  <div className="w-12 h-12 rounded-xl bg-blood flex items-center justify-center shrink-0">
                    <span className="font-mono font-bold text-danger-300">{resp.blood_group}</span>
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <h3 className=" font-semibold text-surface-900">{resp.hospital_name}</h3>
                    ...
                    <p className="text-sm text-surface-500 mt-0.5">{resp.units_needed} unit{resp.units_needed > 1 ? 's' : ''} needed</p>
                    {resp.via_ward && resp.ward_member_name && (
                      <p className="text-xs text-brand-300/80 mt-1 flex items-center gap-1">
                        🏛️ Requested by your ward member: <strong>{resp.ward_member_name}</strong>
                        {resp.ward_member_phone && (
                          <a href={`tel:${resp.ward_member_phone}`} className="text-brand-400 hover:text-brand-300 ml-1">
                            ({resp.ward_member_phone})
                          </a>
                        )}
                      </p>
                    ...
                    {resp.patient_name && (
                      <p className="text-xs text-brand-300/70 mt-1">Patient: {resp.patient_name}</p>
                    )}
                    {resp.patient_condition && (
                      <p className="text-xs text-surface-300 mt-0.5 line-clamp-1">{resp.patient_condition}</p>
                    )}
                    {resp.hospital_latitude && (
                      <div className="flex items-center gap-1.5 mt-1.5 text-xs text-surface-400">
                        ...
                        <a href={`https://maps.google.com/?q=${resp.hospital_latitude},${resp.hospital_longitude}`}
                           target="_blank" rel="noreferrer" className="hover:text-brand-300 transition-colors">
      ...
      {history.length > 0 && (
        <div className="card p-4">
          <h3 className=" font-semibold text-surface-900 mb-3 flex items-center gap-2">
            <Heart size={16} className="text-danger-400"/> Recent Donations
          </h3>
          <div className="space-y-2">
            {history.slice(0, 5).map(h => (
              <div key={h.id} className="flex items-center gap-3 py-2 border-b border-white/[0.05] last:border-0">
                <div className="w-8 h-8 rounded-lg bg-blood flex items-center justify-center text-xs font-mono font-bold text-danger-300">
                  {h.blood_group}
                </div>
                <div className="flex-1">
                  <p className="text-sm text-surface-900">{h.hospital_name}</p>
                  <p className="text-xs text-surface-400">{fmtDate(h.donated_at)}</p>
                </div>
