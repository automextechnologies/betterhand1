  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="section-title flex items-center gap-2"><Calendar size={22} className="text-brand-400"/>Blood Camps</h1>
          <p className="text-surface-400 text-sm mt-0.5">Upcoming community donation drives</p>
        </div>
        {user?.role === 'hospital' && (
      ...
      {showCreate && (
        <div className="card p-5 space-y-4">
          <h3 className=" font-semibold text-surface-900">New Blood Camp</h3>
      ...
      {camps.length === 0 ? (
        <div className="card p-10 text-center">
          ...
          <p className="text-surface-400">No upcoming blood camps.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {camps.map(c => (
            <div key={c.id} className="card p-5 hover:border-brand-700/30 transition-all">
              ...
              <div className="space-y-1.5 mb-4 text-sm text-surface-600">
                <div className="flex items-center gap-2"><Calendar size={13} className="text-brand-400"/>{fmtDate(c.scheduled_date)}</div>
                <div className="flex items-center gap-2"><Clock size={13} className="text-brand-400"/>{c.start_time} — {c.end_time}</div>
                <div className="flex items-center gap-2"><MapPin size={13} className="text-brand-400"/>{c.location}, {c.city}</div>
                <div className="flex items-center gap-2"><Users size={13} className="text-brand-400"/>{c.registered_count}/{c.capacity} registered</div>
              </div>
              {user?.role === 'donor' && (isReg(c.id) ? (
                <button onClick={() => cancel(c.id)}
                  className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-accent-50 text-accent-600 border border-accent-500/30 hover:bg-brand-50 hover:text-danger-500 transition-all text-sm font-semibold">
