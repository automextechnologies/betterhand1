  ...
  if (!donors?.length) {
    return (
      <div className="card p-8 text-center">
        ...
        <p className="text-surface-500 text-sm">Waiting for donor responses…</p>
        <p className="text-surface-900/25 text-xs mt-1">Notifications sent. Check back in a moment.</p>
      </div>
    )
  }
    ...
    <div className="space-y-4">
      ...
      {stats && (
        <div className="grid grid-cols-3 gap-3">
          {[
            { label: 'Accepted', val: stats.total_accepted, color: 'text-accent-400' },
            { label: 'Pending', val: stats.pending_count, color: 'text-yellow-400' },
            { label: 'Rejected', val: stats.rejected_count, color: 'text-danger-400' },
          ].map(s => (
      ...
      <div className="space-y-3">
        {donors.map((d, i) => (
          <div key={d.id} className={clsx('card p-4 border animate-fade-up', 'border-surface-200 hover:border-brand-700/40 transition-all duration-300')}
               style={{ animationDelay: `${i * 80}ms` }}>
            <div className="flex items-start gap-3">
              {/* Rank badge */}
              <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${COLORS[i]} flex items-center justify-center text-surface-900  font-bold text-sm shrink-0`}>
                {LABELS[i]}
              </div>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <h4 className=" font-semibold text-surface-900 truncate">{d.donor_name}</h4>
            ...
            <div className="flex gap-2 mt-3">
              {d.donor_phone && (
                <a href={callLink(d.donor_phone)} className="btn-ghost flex-1 justify-center text-xs py-2 border border-surface-200">
                  <Phone size={13}/> Call
                </a>
              )}
              {d.donor_whatsapp && (
                <a href={whatsappLink(d.donor_whatsapp)} target="_blank" rel="noreferrer"
                   className="btn-ghost flex-1 justify-center text-xs py-2 border border-accent-600/30 text-accent-400 hover:bg-accent-600/10">
