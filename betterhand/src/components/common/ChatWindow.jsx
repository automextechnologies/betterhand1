    ...
    <div className={`card flex flex-col ${className}`}>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 scrollbar-thin min-h-0 max-h-80">
        {loading ? (
          ...
          <p className="text-center text-ink-400 text-sm py-6">No messages yet. Say hello! 👋</p>
        ) : messages.map(m => (
          <div key={m.id} className={`flex ${isMine(m) ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[75%] rounded-2xl px-4 py-2.5 text-sm
              ${isMine(m)
                ? 'bg-gradient-to-r from-brand-600 to-brand-700 text-ink-900 rounded-br-sm'
                : 'bg-white/8 text-ink-900/90 rounded-bl-sm'}`}>
              {!isMine(m) && <p className="text-xs text-ink-400 mb-1 ">{m.sender_name}</p>}
              <p className="leading-relaxed">{m.message}</p>
              <p className={`text-[10px] mt-1 ${isMine(m) ? 'text-ink-600' : 'text-ink-400'}`}>
      ...
      {/* Input */}
      <div className="px-3 py-3 border-t border-ink-200 flex gap-2">
        <input
