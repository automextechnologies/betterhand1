...
function StepDot({ num, label, active, done }) {
  return (
    <div className={`flex items-center gap-2 text-xs  font-semibold
      ${active ? 'text-ink-900' : done ? 'text-emerald-600' : 'text-ink-400'}`}>
      <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-all
        ${active ? 'bg-gradient-to-r from-brand-600 to-brand-700 border-brand-400 text-ink-900 shadow-md'
          : done  ? 'bg-emerald-50 border-accent-500 text-emerald-600'
          : 'bg-surface-50 border-white/15 text-ink-400'}`}>
        {done ? '✓' : num}
      </div>
...
export default function BloodRequestForm({ onSuccess, onClose }) {
  ...
  // Search ward members when location fields filled
  ...
  const doSubmit = async (data) => {
    ...
    try {
      const payload = {
        ...
        ward_contact_person: data.ward_contact_person || '',
        ward_contact_phone:  data.ward_contact_phone  || '',
      ...
      <div className="card w-full max-w-2xl max-h-[92vh] flex flex-col shadow-lg">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-ink-200 shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-xl bg-gradient-to-r from-brand-600 to-brand-700 flex items-center justify-center">
              <Droplets size={16} className="text-ink-900"/>
            </div>
            <h2 className=" font-bold text-ink-900">New Blood Request</h2>
          </div>
          <button type="button" onClick={onClose} className="p-2 rounded-lg hover:bg-surface-100 text-ink-400 hover:text-ink-900 transition-colors">
            <X size={18}/>
          </button>
        </div>

        {/* Step indicators */}
        <div className="flex items-center gap-3 px-6 py-3 border-b border-ink-200 shrink-0">
        ...
        {/* Body — NOT a form tag to prevent accidental submit */}
        <div className="flex-1 overflow-y-auto scrollbar-thin p-6 space-y-4">

          {/* STEP 1: Blood Details */}
          {step === 1 && (
            <div className="space-y-4 animate-fade-up">
              <p className="text-xs text-ink-400 uppercase tracking-wider  flex items-center gap-1.5">
            ...
            <div className="space-y-4 animate-fade-up">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  {errors.patient_name && <p className="text-rose-600 text-xs mt-1">{errors.patient_name.message}</p>}
                </div>
                <div>
                ...
                <div className="col-span-2">
                  ...
                  {errors.patient_condition && <p className="text-rose-600 text-xs mt-1">{errors.patient_condition.message}</p>}
                </div>
                <div>
                  ...
                  {errors.patient_ward_type && <p className="text-rose-600 text-xs mt-1">{errors.patient_ward_type.message}</p>}
                </div>
                <div>
                ...
                <div>
                  <label className="label">Ward Contact Person</label>
                  <input className="input" placeholder="Sister Anu" {...register('ward_contact_person')}/>
                </div>
                <div>
                  <label className="label">Ward Contact Phone</label>
                  <input className="input" placeholder="+91 9876543210" {...register('ward_contact_phone')}/>
                </div>
                <div className="col-span-2">
                  <label className="label">Bystander Mobile Number</label>
                  <input className="input" placeholder="+91 9876543210 (patient's family/bystander)"
                    {...register('bystander_phone')}/>
                  <p className="text-xs text-ink-400 mt-1">Ward members can contact this number for coordination</p>
                </div>
              </div>
            </div>
          )}

          {/* STEP 3: Ward Member — OPTIONAL, stays open */}
          {step === 3 && (
            <div className="space-y-4 animate-fade-up">
              <p className="text-xs text-ink-400 uppercase tracking-wider  flex items-center gap-1.5">
              ...
              {/* Big info box */}
              <div className="p-4 rounded-xl bg-brand-100 border border-brand-700/30">
                <p className="text-sm text-brand-700  font-semibold mb-1">
                  🏛️ Notify a Ward Member?
                </p>
                <p className="text-xs text-brand-600/70 leading-relaxed">
                  ...
                  increasing success rate significantly. <strong className="text-brand-600">This step is optional.</strong>
                </p>
              </div>

              {/* Toggle */}
              <div className="flex items-center justify-between p-3 rounded-xl bg-surface-50 border border-ink-200">
                <div>
                  <p className="text-sm  font-semibold text-ink-900">Enable Ward Member Notification</p>
                  <p className="text-xs text-ink-400 mt-0.5">Toggle on to fill patient home area</p>
                </div>
                <label className="flex items-center gap-2 cursor-pointer">
                  <div className="relative">
                    <input type="checkbox" className="sr-only peer" {...register('notify_ward_members')}/>
                    <div className="w-12 h-6 bg-surface-100 rounded-full peer-checked:bg-gradient-to-r from-brand-600 to-brand-700 transition-all duration-300"/>
              ...
              {notifyWard && (
                <div className="space-y-4 p-4 rounded-xl bg-surface-50 border border-ink-200">
                  <p className="text-xs text-ink-500 flex items-center gap-1.5">
                    <MapPin size={11} className="text-brand-600"/>
                    Patient's Home Ward / Area
                  </p>
                        ...
                        return (
                          <div key={wm.id} onClick={()=>setSelected(sel?null:wm)}
                            className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all
                              ${sel?'border-brand-500 bg-brand-100 shadow-md':'border-ink-200 bg-surface-50 hover:border-ink-200'}`}>
                            <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-sm font-bold text-ink-900 shrink-0 ${sel?'bg-gradient-to-r from-brand-600 to-brand-700':'bg-surface-100'}`}>
                              {wm.ward_number}
                            </div>
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-xs text-ink-400">{wm.district}, {wm.state}</p>
                              {m && (
                                <p className="text-xs text-brand-600 mt-0.5">
                                  👤 {m.full_name} · <Phone size={9} className="inline"/> {m.phone}
                  ...
                  <div>
                    ...
                    <textarea rows="3" className="input resize-none"
                      placeholder="Explain urgency, patient details, any special notes for mobilizing donors…"
        ...
        {/* Footer buttons — outside form, controlled manually */}
        <div className="px-6 py-4 border-t border-ink-200 bg-white backdrop-blur-sm flex gap-3 shrink-0">
          {step > 1 ? (
