...
class CollegeListView(APIView):
    """GET /api/accounts/colleges/?district=X — distinct college names for autocomplete."""
    permission_classes = [AllowAny]

    def get(self, request):
        from .models import DonorProfile
        qs = DonorProfile.objects.exclude(college_name='').values_list('college_name', 'college_district').distinct()
        district = request.query_params.get('district', '').strip()
        colleges = []
        seen = set()
        for name, dist in qs:
            if district and dist and district.lower() != dist.lower():
                continue
            key = name.strip().lower()
            if key and key not in seen:
                seen.add(key)
                colleges.append({'name': name, 'district': dist})
        return Response({'colleges': sorted(colleges, key=lambda x: x['name'])})


class QuestionnaireView(APIView):
    """POST /api/accounts/donor/questionnaire/ — save blood donor questionnaire & consent."""
    permission_classes = [IsAuthenticated, IsDonor]
    ...
    def post(self, request):
        ...
        for f in ['q_weight_ok','q_age_ok','q_no_illness','q_no_medication',
            ...
            setattr(dp, f, bool(d.get(f, False)))
        dp.q_chronic_conditions = d.get('q_chronic_conditions', '')
        if d.get('q_last_donation_date'):
            dp.q_last_donation_date = d['q_last_donation_date']
        dp.questionnaire_completed = True
        if dp.consent_given and not dp.consent_date:
            dp.consent_date = timezone.now()
        dp.save()
        return Response({'message': 'Questionnaire saved. You now have priority status!',
