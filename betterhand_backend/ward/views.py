...
class WardDashboardView(APIView):
    ...
    def get(self, request):
        ...
        # STRICT ward match — only donors in THIS exact ward
        ward_q = dict(
            ward_number__iexact=str(ward.ward_number),
            local_body_name__iexact=ward.local_body_name,
            state__iexact=ward.state,
        )
        total_donors = DonorProfile.objects.filter(**ward_q).count()
        avail_donors = DonorProfile.objects.filter(**ward_q, is_available=True).count()
...
class WardDonorsListView(APIView):
    """GET /api/ward/donors/ — all donors in this ward member's area."""
    permission_classes = [IsAuthenticated, IsWardMember]

    def get(self, request):
        member = request.user.ward_member_profile
        ward   = member.ward
        # STRICT ward match — only donors registered in THIS exact ward
        # (ward_number + local_body_name + state). No district-wide fallback,
        # otherwise donors from other wards in the same district would appear.
        ...
        ).order_by('-questionnaire_completed', '-is_available', 'full_name')

        bg = request.query_params.get('blood_group', '').strip()
        if bg: donors = donors.filter(blood_group=bg)
        if request.query_params.get('available', '').lower() == 'true':
        ...
        for dp in donors:
            ...
            result.append({
                'id': dp.id, 'full_name': dp.full_name, 'blood_group': dp.blood_group,
                'phone': dp.phone, 'whatsapp_number': dp.whatsapp_number,
                'whatsapp_link': f'https://wa.me/{digits}' if digits else None,
                'age': dp.age, 'gender': dp.gender, 'is_available': dp.is_available,
                'district': dp.district or dp.city, 'ward_number': dp.ward_number,
                'is_student': dp.is_student, 'college_name': dp.college_name,
                'questionnaire_completed': dp.questionnaire_completed,
                'priority': dp.priority_score,
        ...
        return Response({
            'ward': {'ward_number': ward.ward_number, 'local_body_name': ward.local_body_name,
                     'district': ward.district, 'state': ward.state},
            'total': len(result),
    ...
    def get(self, request, pk):
        ...
        for dp in candidates:
            ...
            scored.append({
                ...
                'donation_count': dp.donation_count or 0,
                'priority':       dp.questionnaire_completed,
                'badges':         list(dp.user.badges.values_list('badge', flat=True)),
                'whatsapp_link':  f'https://wa.me/{digits}' if digits else None,
            })

        scored.sort(key=lambda d: (
            0 if d.get('priority') else 1,   # questionnaire-completed donors first
            d['distance_km'],
            -(d['avg_rating'] or 0),
            -d['donation_count']
        ))
        ...
        return Response({
            ...
            'bystander_phone':  (alert.blood_request.bystander_phone if alert.blood_request else ''),
