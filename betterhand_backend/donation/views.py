...
def _promote_next_donor(br, exclude_ids):
    """Find next best accepted donor, confirm them, notify. Returns the response or None."""
    nxt = br.next_best_accepted(exclude_ids=exclude_ids)
    if not nxt:
        return None
    ...
    try:
        from .notification_service import send_push_notification
        if nxt.donor.fcm_token:
            send_push_notification(nxt.donor.fcm_token, 'You are CONFIRMED!',
                f'{hospital_name} needs you now. Head to the hospital!',
                {'type':'donation_confirmed','response_id':str(nxt.id)})
    except Exception: pass
    try:
        from channels.layers import get_channel_layer
        from asgiref.sync import async_to_sync
        async_to_sync(get_channel_layer().group_send)(
            f'donor_{nxt.donor_id}',
            {'type':'donation.event','event_type':'donation_confirmed',
             'payload':{'response_id':nxt.id,'hospital_name':hospital_name,
                        'hospital_latitude':str(br.hospital_latitude or ''),
                        'hospital_longitude':str(br.hospital_longitude or '')}})
    except Exception: pass
    return nxt
...
class NoDonationView(APIView):
    """Mark confirmed donor as arrived-no-donation (NO cooldown), auto-promote next."""
    permission_classes = [IsAuthenticated, IsHospital]
    ...
    def post(self, request, pk):
        ...
        resp.save(update_fields=['status'])
        br = resp.request
        confirmed_ids = list(br.responses.filter(status='confirmed').values_list('id', flat=True))
        promoted = _promote_next_donor(br, exclude_ids=confirmed_ids + [resp.id])
        msg = 'Marked arrived (no donation, no cooldown).'
        if promoted:
            try: msg += f' Next donor {promoted.donor.donor_profile.full_name} auto-confirmed.'
            except Exception: msg += ' Next donor auto-confirmed.'
        return Response({'message': msg, 'promoted': bool(promoted)})


class CancelResponseView(APIView):
    """Hospital cancels a donor (no-show), auto-promote next best accepted donor."""
    permission_classes = [IsAuthenticated, IsHospital]
    ...
    def post(self, request, pk):
        ...
        try:
            from .notification_service import send_push_notification
            if resp.donor.fcm_token:
                send_push_notification(resp.donor.fcm_token, 'Request Update',
                    'The hospital has cancelled your assignment.', {'type':'cancelled'})
        except Exception: pass
        promoted = None
        if was_confirmed:
            confirmed_ids = list(br.responses.filter(status='confirmed').values_list('id', flat=True))
        ...
        msg = 'Donor cancelled.'
        if promoted:
            try: msg += f' {promoted.donor.donor_profile.full_name} auto-promoted as replacement.'
            except Exception: msg += ' Next donor auto-promoted.'
        return Response({'message': msg, 'promoted': bool(promoted)})

class ClearDataView(APIView):
    """POST /api/donation/clear-data/ — Delete all donation data for fresh testing."""
    permission_classes = [IsAuthenticated, IsHospital]

    def post(self, request):
        from ward.models import WardBloodAlert, WardDonorNotification
        # Only clear this hospital's data
        requests_qs = BloodRequest.objects.filter(hospital=request.user)
        response_ids = DonationResponse.objects.filter(request__in=requests_qs).values_list('id', flat=True)
        ChatMessage.objects.filter(response_id__in=response_ids).delete()
        DonationRecord.objects.filter(request__in=requests_qs).delete()
        WardDonorNotification.objects.filter(alert__blood_request__in=requests_qs).delete()
        WardBloodAlert.objects.filter(blood_request__in=requests_qs).delete()
        DonationResponse.objects.filter(request__in=requests_qs).delete()
        count = requests_qs.count()
        requests_qs.delete()
        return Response({'message': f'Cleared {count} blood requests and all related data.'})
